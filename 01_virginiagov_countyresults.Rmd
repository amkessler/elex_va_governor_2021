...
Nice. This is giving us some pretty long column names. we can change them after the fact using rename(). but first let's clean_names to make it easier
```{r}
...
The percentage columns are actually text values, not numbers. And they have that % sign in the text too.
Let's fix that using a handy function from the readr package, parse_number().
...
Few quick things to deal with...  first let's get rid of the total votes in 2021, we don't need that anymore.
But alos the locality isn't the same case as in our governor's results, so let's uppercase it.
```{r}
...
Great. Now we're all ready to join
